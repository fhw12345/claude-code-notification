...
function buildFlashScript(): string {
  return [
    ...
    "  if ($terminalProcs) { foreach ($tp in $terminalProcs) { if (-not $candidatePids.Contains($tp.Id)) { $candidatePids.Add($tp.Id) | Out-Null } } }",
    "  if ($candidatePids.Count -eq 0) {",
    "    $codeProcs = Get-Process -Name 'Code' -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne [IntPtr]::Zero };",
    "    if ($codeProcs) { foreach ($cp in $codeProcs) { if (-not $candidatePids.Contains($cp.Id)) { $candidatePids.Add($cp.Id) | Out-Null } } }",
    "  }",
    "}",
